...
async def main():
    ...
    # --- power_action always gates (no preauth bypass in this project) ---
    calls.clear()
    tools_system.configure(confirm=deny)
    assert (await tools_system.power_action({"action": "sleep"})).startswith("User DENIED")
    assert "power" not in calls
    print("PASS power_action always gates (sleep denied -> nothing ran)")
